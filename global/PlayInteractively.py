...
class PlayInteractively():  #Controller
    ...
    def __init__(self,dataset_name='ffhq'):
        ...
        self.view.tester.bind('<ButtonPress-1>', self.DT2_tester)
...
#        
    ...
    def DT2_tester(self,event):
        if( self.gravy == False):
            self.prevholder= self.style_clip.GetCode()
            self.gravy = True
        else:
    ...
    def ChangeAlpha(self,event):
        ...
        img=self.style_clip.GetImg()
        print(str(self.style_clip.M.alpha))
        print('manipulate one Alpja')
        img=Image.fromarray(img)
        img = ImageTk.PhotoImage(img)
        self.addImage_m(img)
...
if __name__ == "__main__":
    ...
    parser2.add_argument('--dataset_name',type=str,default='ffhq',
